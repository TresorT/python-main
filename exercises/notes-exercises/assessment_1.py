...
for i in range(7):
    ...
    if player_one_cards[i] > player_two_cards[i]:
        print('Round number', turn, ':Player 1 wins the round, with', player_one_cards[i], ' beating ',
              player_two_cards[i])
        player_one_score += 1
    elif player_one_cards[i] < player_two_cards[i]:
        print('Round number', turn, ':Player 2 wins the round, with', player_two_cards[i], ' beating ',
              player_one_cards[i])
        player_two_score += 1
    else:
